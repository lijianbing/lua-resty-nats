...
static int ngx_http_lua_socket_tcp_setkeepalive_nats(lua_State *L);
...
static ngx_int_t ngx_http_lua_socket_keepalive_close_handler_nats(ngx_event_t *ev);
static void ngx_http_lua_socket_keepalive_rev_handler(ngx_event_t *ev);
static void ngx_http_lua_socket_keepalive_rev_handler_nats(ngx_event_t *ev);
...
ngx_http_lua_inject_socket_tcp_api(ngx_log_t *log, lua_State *L)
{
    ...
    lua_pushcfunction(L, ngx_http_lua_socket_tcp_setkeepalive_nats);
    lua_setfield(L, -2, "setkeepalive_nats");
...
static void
ngx_http_lua_socket_keepalive_rev_handler_nats(ngx_event_t *ev)
{
    (void) ngx_http_lua_socket_keepalive_close_handler_nats(ev);
}
...
static ngx_int_t
ngx_http_lua_socket_keepalive_close_handler_nats(ngx_event_t *ev)
{
    ngx_http_lua_socket_pool_item_t     *item;
    ngx_http_lua_socket_pool_t          *spool;
    ...
    if (n==1) {
        ...
        n = c->recv(c, d_buf, 10);
        if (n>0) {
            if (ngx_strncasecmp(d_buf, (u_char *) "PING", 4) == 0) {
                ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ev->log, 0, "internal error processing NATS message PING");
                n = c->send(c, (u_char *) "PONG\r\n", 6);
                if (n == 6) {
                    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ev->log, 0, "internal error processing NATS message PONG");
                }
            }
        }
        return NGX_OK;
